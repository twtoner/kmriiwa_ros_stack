...
#include <kmriiwa_gazebo/gazebo_ros_moveit_planning_scene.h>
      ...
      ++model_it)
  {
        ...
        ++link_it)
    {
      ...
      if(found_collision_object == collision_object_map_.end() || publish_full_scene_){
        ...
        // new_object.id = id;    // replaced with the following if-else from (https://answers.ros.org/question/379844/error-while-grasp-planning-using-moveit-with-collision-objects-originally-spawned-into-gazebo-world/)
        if (model_name != id)   
        ...
        new_object.header.frame_id = "kmriiwa_odom";
